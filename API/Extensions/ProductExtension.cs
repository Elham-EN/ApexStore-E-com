...
public static class ProductExtension
{
        ...
        string? brands, string? types)
    {
        ...
        {
            brandList
                .AddRange([.. brands.ToLower().Split(",")]);
        }

        if (!string.IsNullOrEmpty(types))
        {
            typeList
                .AddRange([.. types.ToLower().Split(",")]);
        }

        // Query for products that contain that brand
        query = query.Where(x => brandList.Count == 0 ||
                    brandList.Contains(x.Brand.ToLower()));

        // Query for products that contain that type
        query = query.Where(x => typeList.Count == 0 ||
                    typeList.Contains(x.Type.ToLower()));
